...
def show_staff_dashboard():  # 👈 MAIN ENTRY FUNCTION
    ...
    menu = ["Insert Student","Insert fees" ,"Update Details", "Delete Student", "View Records", "Search by SPR", "Search by Date"]
    ...
    if choice == "Insert Student":
        ...
        tab1, tab2, tab3 = st.tabs(["🎓 Students", "💳 Payments", "📊 Combined View"])
    ...
    elif choice == "Search by SPR":
        
        st.subheader("🔍 Search Record by SPR No")
        spr_no = st.number_input("Enter SPR No", step=1)

        if st.button("Search"):
            # Fetch data only when Search is clicked
        ...
        st.subheader("📅 Filter Records by Bill Date")
        col1, col2 = st.columns(2)
        with col1:
        ...
        if st.button("Fetch Records"):
            ...
            try:
                #display_table(res.json()["PAYMENT DETAILS"], f"Payments from {from_date} to {to_date}")
                data = res.json().get("PAYMENT DETAILS", [])
                    #display_table(data.get("PAYMENT DETAILS", []), "Payment Records")
                columns =  [
                        "SPR_NO", "NAME", "DEPT", "PH_NO", "TUSION_FEES", "HOSTEL_FEES",
